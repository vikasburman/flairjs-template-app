...
const build = (options, done) => {
    ...
    const resolveRoot = (buildPath) => {
        ...
        if (options.customBuildConfig.resolveRoot[buildPath]) {
            resolvedRoot = options.customBuildConfig.resolveRoot[buildPath];
        } else {
            for(let partialBuildPath in options.customBuildConfig.resolveRoot) {
                if (options.customBuildConfig.resolveRoot.hasOwnProperty(partialBuildPath)) {
                    if (buildPath.startsWith(partialBuildPath)) {
                        resolvedRoot = options.customBuildConfig.resolveRoot[partialBuildPath];
                        break;
                    }
                }
            }
    ...
    const resolvePreamble = (buildPath) => {
        ...
        if (options.customBuildConfig.preambleRoot[buildPath]) {
            preambleRoot = options.customBuildConfig.preambleRoot[buildPath];
        } else {
    ...
    const createPreamble = () => {
        ...
        logger(0, 'preamble', options.current.preamble.replace(options.current.dest, options.current.resolvedRoot), true);
    ...
    const processAssemblies = (done) => {
        ...
        options.current.asmFileName = options.current.asm.replace(options.current.dest, options.current.resolvedRoot);
    ...
    const processSources = (done) => {
        ...
        options.current.skipMinify = options.customBuild ? (options.customBuildConfig.skipMinify.indexOf(source) !== -1) : false;
 ...
 *                  "skipMinify": [ ] - some of the build folders may be skipped for minify (e.g., server folders)
 *                                      those can be defined here
 *                  "resolveRoot": {
 *                      "<buildPath>": "<resolveWithRoot>" - for each path in "build", define how this will be resolved when generating path of assemblies underneath
 *                  }
 *                      e.g., "server/app/": "./app/" - means, assembly file names will be like ./app/file.js
 ...
 *                  }
 *                      e.g., "server/app/": "server/app/" - means, assembly files that exists under server/app/, will have their preamble generated at: server/app/preamble.js
